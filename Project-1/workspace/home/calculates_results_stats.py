...
def calculates_results_stats(results_dic):
    ...
    for filename, value in results_dic.items():
        ...
        if not is_dog and not classified_as_dog:
            n_correct_notdogs += 1
        if is_dog and match_real_classified:
            n_correct_breed += 1
        if match_real_classified:
            n_match += 1
...
# This was my first solution but I decided, that it is no so readble as the solution that I used in the code above
# But I still decided to leave it here:)
# 
#     is_dog_img = 3
#     is_dog_classifier = 4
#     match = 2
#     results = results_dic.values()
    
#     n_correct_dogs = sum(
#         [dogs[is_dog_img] for dogs in results if dogs[is_dog_img] and dogs[is_dog_classifier]]
#     )
#     n_dogs_img = sum([dogs[is_dog_img] for dogs in results])
#     n_correct_notdogs = sum(
#         [nondogs[is_dog_img] for nondogs in results if not nondogs[is_dog_img] and not nondogs[is_dog_classifier]]
#     )
#     n_notdogs_img = n_images - n_dogs_img
#     n_correct_breed = sum(
#         [dogs[is_dog_img] for dogs in results if dogs[is_dog_img] and dogs[match]]
#     )
#     n_match = sum(dogs[match] for dogs in results)